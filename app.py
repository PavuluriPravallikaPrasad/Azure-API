...
# Load API Key from environment variable (or replace with a hardcoded key)
...
def verify_api_key():
    ...
    api_key = request.headers.get("x-api-key")  # Custom header for API key
...
class UserResource(Resource):
    def post(self):
        if not verify_api_key():
            return jsonify({"message": "Unauthorized Access"}), 403  # 403 Forbidden
...
class GetUserResource(Resource):
    def get(self):
        ...
        try:
            ...
            users_data = [{"first_name": u.first_name, "last_name": u.last_name, "email": u.email,
                           "username": u.username, "contact_number": u.contact_number, "address": u.address} 
                          for u in users]
...
api.add_resource(UserResource, "/user")  # POST (Create User)
api.add_resource(GetUserResource, "/users")  # GET (Retrieve Users)
